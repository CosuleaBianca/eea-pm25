...
OUTPUT_METRICS_PATH = "results/lr_dl_metrics.csv"
OUTPUT_PREDS_PATH = "results/lr_dl_predictions_sample.csv"
OUTPUT_RUN_INFO_JSON = "train_models/global/runs_info/lr_dl_run_info.json"
OUTPUT_MODEL_DIR = "models/lr_dl_models"
...
# ... (funcțiile build_multi_horizon_targets, select_feature_columns, recover, calculate rămân neschimbate) ...
# Pentru claritate, le omit aici, dar ele trebuie să existe în codul final.
# Presupunem că sunt definite mai sus.
...
def main():
    ...
    for h in HORIZONS:
        ...
        train_h_subset = filter_df_by_anchors(train_h_subset, train_anchors_by_h.get(h))
        test_h_subset = filter_df_by_anchors(test_h_subset, test_anchors_by_h.get(h))

        if len(train_h_subset) == 0 or len(test_h_subset) == 0:
    ...
    # ... (Restul codului de metrici rămâne identic) ...
...
if __name__ == "__main__":
    # Asigură-te că funcțiile lipsă sunt definite deasupra
    main()
