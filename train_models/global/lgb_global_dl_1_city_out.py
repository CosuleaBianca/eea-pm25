...
EXCLUDE_COUNTRY = "AT"  # Change to "FR" or other country code to test different cities

OUTPUT_METRICS_PATH = f"results/lgb_dl_metrics_tuned_exclude_{EXCLUDE_COUNTRY}.csv"
OUTPUT_PREDS_PATH = f"results/lgb_dl_predictions_sample_exclude_{EXCLUDE_COUNTRY}.csv"
OUTPUT_BEST_PARAMS_JSON = f"train_models/global/lgb_dl_best_params_by_horizon_exclude_{EXCLUDE_COUNTRY}.json"
OUTPUT_RUN_INFO_JSON = f"train_models/global/runs_info/lgb_dl_run_info_exclude_{EXCLUDE_COUNTRY}.json"

MODEL_DIR = f"models/models_lgb_dl_exclude_{EXCLUDE_COUNTRY}"
...
# Setăm un n_estimators mare și lăsăm early stopping să decidă best_iteration
...
def tune_hyperparameters(train_df_base: pd.DataFrame, feature_cols: list[str],
                         horizons: list[int], sampler: list,
                         train_anchors_by_h: Optional[dict[int, pd.DataFrame]] = None) -> dict:
    """
...
def main():
    ...
    print(f">>> EXCLUDED COUNTRY: {EXCLUDE_COUNTRY}")
    ...
    # Build LSTM-aligned anchors on FULL data (before exclusion)
    # This ensures excluded country gets anchors too
    ...
    # For leave-one-city-out: separate train and test stations
    # Train stations: common_stations without excluded country
    # Test stations: all stations in test set (including excluded country)
    train_stations = [s for s in common_stations if not s.startswith(f"{EXCLUDE_COUNTRY}_")]
    test_stations = set(df[df["dt_utc"] >= TEST_START]["station_key"].unique())
    all_stations = sorted(set(train_stations) | test_stations)

    # Filter to all stations (not just common)
    df = df[df["station_key"].isin(all_stations)].copy()
    print(f"LSTM-aligned stations: {len(common_stations)} total | {len(train_stations)} for training (excluding {EXCLUDE_COUNTRY})")
    print(f"Test stations: {len(test_stations)} (includes all cities)")
    ...
    # LEAVE-ONE-CITY-OUT: Exclude country from training
    # =========================
    train_before = len(train_df_base)
    train_df_base = train_df_base[train_df_base["Country"] != EXCLUDE_COUNTRY].copy()
    ...
    if os.path.exists(OUTPUT_BEST_PARAMS_JSON):
        ...
        best_params_by_h = tune_hyperparameters(
            train_df_base,
            feature_cols,
            HORIZONS,
            sampler,
    ...
    print("\n=== Sample of Results (Global LGB DL - Leave-One-City-Out) ===")
