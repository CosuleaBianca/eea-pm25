...
# Set to None to train on all cities
EXCLUDE_COUNTRY = "FR"  # Change this to test different cities

# Output paths include excluded country
OUTPUT_METRICS_PATH = f"results/gam_metrics_tuned_exclude_{EXCLUDE_COUNTRY}.csv"
OUTPUT_PREDS_PATH = f"results/gam_predictions_sample_exclude_{EXCLUDE_COUNTRY}.csv"
OUTPUT_BEST_PARAMS_JSON = f"train_models/global/gam_best_params_by_horizon_exclude_{EXCLUDE_COUNTRY}.json"
OUTPUT_RUN_INFO_JSON = f"train_models/global/runs_info/gam_run_info_exclude_{EXCLUDE_COUNTRY}.json"

MODEL_DIR = f"models/models_gam_exclude_{EXCLUDE_COUNTRY}"
...
def main():
    ...
    # =========================
    # LEAVE-ONE-CITY-OUT: Remove excluded country from TRAINING only
    # =========================
    ...
    if EXCLUDE_COUNTRY is not None:
        ...
        print("\n" + "="*70)
        print(f">>> LEAVE-ONE-CITY-OUT: Excluded {EXCLUDE_COUNTRY} from ALL training (HPO + final)")
        ...
        # Count how many test samples are from excluded country
    ...
    # =========================
    # Train models for each horizon (WITHOUT excluded city)
    # =========================
    for h in HORIZONS:
        h_t0 = time.time()
        print(f"\n================= HORIZON h={h} =================")
        ...
        # Train final pe tot TRAIN (<=2022, WITHOUT excluded city)
        ...
        # test predict (on ALL cities including excluded)
