...
# =========================
# CONFIG (split_2: train 2023-2024H1, test 2024H2)
# =========================
CSV_PATH = "ml_ready_dataset_full_realistic.csv"

OUTPUT_METRICS_PATH = "results/lgb_split_2_metrics_tuned.csv"
OUTPUT_PREDS_PATH = "results/lgb_split_2_predictions_sample.csv"
OUTPUT_BEST_PARAMS_JSON = "train_models/global/lgb_split_2_best_params_by_horizon.json"
OUTPUT_RUN_INFO_JSON = "train_models/global/runs_info/lgb_split_2_run_info.json"

MODEL_DIR = "models/models_lgb_split_2"
...
# Train/Test (split_2: train 2023-2024H1, test 2024H2)
TRAIN_END = pd.Timestamp("2024-06-30 23:00:00", tz="UTC")
TEST_START = pd.Timestamp("2024-07-01 00:00:00", tz="UTC")
TEST_END = pd.Timestamp("2024-12-31 23:00:00", tz="UTC")

# Internal split for tuning (within TRAIN)
TUNE_TRAIN_END = pd.Timestamp("2024-03-31 23:00:00", tz="UTC")
TUNE_VAL_START = pd.Timestamp("2024-04-01 00:00:00", tz="UTC")
TUNE_VAL_END = pd.Timestamp("2024-06-30 23:00:00", tz="UTC")
...
def tune_hyperparameters(train_df_base: pd.DataFrame, feature_cols: list[str],
    ...
    for h in horizons:
        ...
        if not use_internal_val:
            ...
            print(f"[Tuning] Holdout: train<=2024Q1 ({len(tune_train):,}) | val=2024Q2 ({len(tune_val):,})")
            ...
            print(f"[Tuning] Done in {tune_sec:.1f}s | best MAE (holdout 2024Q2) = {best_mae:.4f}")
...
def main():
    ...
    run_t0 = time.time()
    print(">>> [LGB Fast Tuned - Split 2] Loading CSV...")
    ...
    for h in HORIZONS:
        ...
        # =========================
        # Train final pe tot TRAIN (<=2024H1) cu early stopping pe 2024Q2 (dacă există)
        # =========================
        ...
        # save model
        model_path = os.path.join(MODEL_DIR, f"lgb_split_2_h{h}.pkl")
    ...
    print("\n=== Sample of Results (Global LGB Fast Tuned - Split 2) ===")
