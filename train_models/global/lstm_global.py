...
OUTPUT_METRICS_PATH = "results/lstm_global_metrics.csv"
OUTPUT_PREDS_PATH = "results/lstm_global_predictions_sample.csv"
OUTPUT_RUN_INFO_JSON = "train_models/global/runs_info/lstm_global_run_info.json"
MODELS_DIR = "models/lstm_global_models"
HIST_DIR = "train_models/history/lstm_global_histories"
SCALERS_JSON = "train_models/scalers/lstm_global_station_scalers.json"
...
def build_single_model_direct(n_features: int, n_stations: int) -> Model:
    ...
    # direct head: predict PM2.5(t+h) (normalized)
    d = layers.Dense(16, activation="relu",
                     kernel_regularizer=tf.keras.regularizers.l2(1e-4))(x)
    d = layers.Dropout(0.20)(d)
    y_hat = layers.Dense(1, name="y_hat")(d)
...
# ==========================================================
def build_examples_for_segment_single_direct(
    seg: pd.DataFrame,
    station_idx: int,
    ...
    for t in range(start_t, end_t):
        ...
        if require_no_nan and np.isnan(y_norm):
            continue
        ...
        for station_key, g in train_build_df.groupby("station_key", sort=False):
            ...
            for seg in segs:
                if len(seg) < MIN_SEG_LEN:
                    continue
                Xs, sids, ys, _ = build_examples_for_segment_single_direct(
...
def make_test_generator_for_h(
    ...
    for station_key, g in test_build_df.groupby("station_key", sort=False):
        ...
        for seg in segs:
            if len(seg) < MIN_SEG_LEN:
                continue
            Xs, sids, ys, Ts = build_examples_for_segment_single_direct(
...
def main():
    ...
    scalers = fit_station_scalers(train_df.dropna(subset=feature_cols), feature_cols)
    ...
    for h in HORIZONS:
        print("\n" + "=" * 70)
        print(f"Training single-output DIRECT LSTM for horizon h={h}...")
        print("=" * 70)
        ...
        model = build_single_model_direct(n_features=len(feature_cols), n_stations=n_stations)
        ...
        model_path = os.path.join(MODELS_DIR, f"lstm_direct_h{h}.keras")
